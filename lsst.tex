...
\slugcomment{A living LSST document (arXiv:0805.2366); version 3.0.2 of July 27, 2014}
...
http://www.lsst.org/lsst/news/aas\_215 and http://www.lsst.org/lsst/news/aas\_217}. 
...
least two bands, and excellent photometry in all bands to measure photometric redshifts 
(a requirement shared with BAO). The strongest constraints on the LSST image quality arise 
from this science program. In order to control systematic errors in shear measurement, 
...
techniques show that, in order to obtain a sample of $\sim$4 billion lensing galaxies,
...
Type Ia supernovae (SN) provided the first robust evidence that the expansion of the
...
but will lead to insights into planetary formation and evolution by providing the basis and constraints 
...
asteroids or PHAs, are in orbits that pass sufficiently close to Earth’s orbit, to within 0.05 AU, that perturbations 
...
implement a NEO survey that would catalog 90\% of NEOs larger than 140 meters by 2020.  
...
models and enable prediction of occulations. The photometry should be better than 1-2\% to enable color-based 
taxonomy of the asteroids, and preferably obtained in multiple filters in a short time span to reduce apparent 
variations in color due to changes in observing geometry, while still obtaining photometry over many lunations 
in order to determine phase curves and allow shape modeling. 

In order to fulfill the Congressional mandate using a ground-based facility, a 10-meter 
class telescope equipped with a multi-gigapixel camera, and a sophisticated 
and robust data processing system are required (Ivezi\'{c} et al. 2007a). The images should reach a depth of at 
least 24.5 (5$\sigma$ for point sources) in the $r$ band to fulfill the Congressional NEO mandate, and to probe 
the $\sim$100 m size range at main-belt distances and study distant TNOs, including discovering rare distant 
TNOs such as Sedna (Brown, Trujillo \& Rabinowitz 2004) and 2012 VP113 (Trujillo \& Sheppard 2014).
...
al. 2009) and the Palomar Transient Factory (Law et al. 2009). Decade-long cadence would enable the study of long period 
variables and quasars (Kaspi et al. 2007; MacLeod et al. 2010; Graham et al. 2014). It should be possible to measure colors of fast 
...
et al 2013). The Milky Way and its environment provide a unique data set for understanding the detailed processes that 
shape galaxy formation and for testing the small-scale predictions of our standard cosmology. However, we still lack 
...
Key requirements for mapping the Galaxy are large area coverage, excellent image quality to maximize the photometric and astrometric accuracy, especially in crowded fields; photometric precision of at least 1\% to separate main sequence and
giant stars (e.g., Helmi et al. 2003) as well as identify variable stars such as RR Lyrae (Sesar et al 2010, Sharma et al 2011); 
...
numerous main-sequence stars, the total coadded depth must reach $r > 27$, with a similar depth in the $g$ band.   
Photometric metallicities can be used to study the metallicity distribution of stars in the Sgr tidal stream (e.g., see 
Majewski et al. 2003, Chou et al. 2007) and other halo substructures ($\sim 30$ kpc, Carollo et al. 2007).  These 
...
within 10 km s$^{-1}$ needed to be competitive with large-scale radial velocity surveys, the required proper motion 
accuracy is at least 0.2 mas yr$^{-1}$. This is the same accuracy as will be delivered by the recently launched Gaia mission\footnote{http://sci.esa.int/gaia/} (Perryman et al. 2001; de Bruijne 2012) at its faint limit ($r \sim 20$). 
In order to produce a complete sample of solar neighborhood stars out to a distance of 300 pc (the thin disk scale height), 
...
accuracy of 10 mas per observation per coordinate, approximately 1,000 observations are required. This requirement on 
the number of observations is in good agreement with the independent constraint implied by the difference between 
the total depth and the single-visit depth.
...
3 year-long survey and 12 m for a 5-year long survey) and not as effective proper motion
...
instrumentation, including a 1.5 m calibration telescope, will provide the 
...
Paul-Baker Mersene-Schmidt system; Angel, Lesser \& Sarlot 2000) with a very fast $f$/1.234 beam. The optical 
...
correct for the chromatic aberrations induced by the necessity of a thick dewar window and flattens the
...
single monolithic blank using spin-cast borosilicate technology. The secondary mirror is being fabricated from 
...
and the primary-tertiary mirror pair is currently being polished at the Steward Observatory Mirror Lab and is 
...
Mamajek 2012).  The telescope enclosure houses a compact, stiff telescope structure atop a 15 m high concrete pier
with a fundamental frequency of 8 Hz, that is crucial for achieving the required fast slew-and-settle times
(see Fig.~\ref{Fig:telescope}).  The height of the pier was set to place the telescope above the degrading 
effects of the turbulent ground layer.  Capping the telescope enclosure is a 30 m diameter dome that has 
a well ventilated designed to reduce dome seeing (local air turbulence that can distort images) 
...
%\includegraphics[width=1.55\hsize,angle=90.0,clip]{camera2009.pdf}
\hskip -0.15in
\includegraphics[width=0.55\textwidth]{CameraFigureUpdated.pdf}
...
side of eq.~\ref{m5}
\begin{equation}
 \Delta m_5^{\rm trailing} = 1.25\,\log_{10}(1+0.028{v \,t_{vis} \over \theta}),
\end{equation}
...
\item Discovery and characterization of thousands of hot Jupiters in exoplanetary systems (Wright et al. 2012). 
...
astronomy by setting the context within which we interpret these broader data sets. Moreover, by providing a 
census of faint satellites and stellar streams in the halo, this map will offer a unique means to constrain the
particle nature of dark matter because candidate supersymmetric particle dark matter models predict different 
mass clustering on small scales, with a corresponding range of mass profiles in low-mass systems.

The LSST will produce a massive and exquisitely accurate photometric and astrometric data set for about 10 billion 
Milky Way stars (20 billion in total). The coverage of the Galactic plane will yield data for numerous star-forming 
...
existing or planned survey will provide such a powerful dataset to study the outer halo, including Gaia
which is flux limited at $r=20$, and the Dark Energy Survey (Rossetto et al.~2011) and Pan-STARRS which both 
lack the $u$ band (which is necessary for estimating metallicity). The LSST in its standard surveying mode will 
...
signatures of our Galaxy's formation (Johnston et al. 200;  Cooper et al. 2010).   LSST will be the only survey 
...
\item The depth and baseline of the survey will allow the measurement of the internal motions of Milky Way dwarf 
          galaxies, thereby constraining their density profiles and the possibly nature of dark matter (e.g., Walker \& Pe{\~n}arrubia 2011).
...
(see http://www.sdss.org). Over 60\% of the more than $\sim$3000 refereed papers based 
...
Periodic calls for applications for membership are issued to the professional 
community at large\footnote{See http://www.noao.edu/lsst/collab\_prop/Scicollab.htm}.
It is anticipated that the LSST science collaborations will perform their analyses using 
...
and {\$37M} per year for operations (in 2014 U.S. dollars). This budget includes a significant
EPO program. Beyond the improved image quality, LSST will be much more cost 
...
available worldwide.  As of 2014, scientists in 67 institutions from 28 countries have open 
access to the data. The software which creates the LSST database will be open source. 
{\it LSST will  be a significant milestone in the globalization of the information revolution.}
