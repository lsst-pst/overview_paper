...
Data collected by the LSST telescope and camera will be automatically processed to {\em data products} -- catalogs and reduced images -- by the LSST Data Management system (\S~\ref{sec:dm}). These products are designed to be sufficient to enable a large majority of LSST science cases. The LSST data products are described in the LSST Data Products Definition Document (DPDD; Juric et al. 2013), so only a high-level overview will be presented here; the reader is instructed to consult the DPDD for details.
