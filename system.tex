...
energy (see e.g. \citet{2006JCAP...08..008Z}, and references therein). The challenge is to turn this available precision into accuracy, by careful modeling and marginalization over a variety of systematic effects \citep[see e.g.,][]{2017MNRAS.470.2100K}.
...
emission from dwarf satellite
galaxies (see e.g., \citet{2014ApJ...795L..13H}
...
Survey (DES) team, which has been approaching 5 mmag calibration
precision \citep{2017arXiv170601542B}.
...
$\sim$6.4 m in diameter. The collected light is reflected to a 3.4 m convex secondary, then onto
...
installed. The calibration telescope building is on the left with its
...
approximately 15 TB per night of raw imaging data\footnote{For
  ...
  approximately 16 TB.}. As with all large modern surveys, the large
data volume, the 
...
\item Periodically process the accumulated survey data to provide a
  ...
  self-consistent data set suitable for use in performing scientific
...
The DM system components will span four key facilities on three
...
community), the Large Data Facility at the National Center
for Supercomputing Applications (NCSA) in Champaign, Illinois, and
CC-IN2P3 in Lyon, France.
All real-time data processing and half the data release product
processing will take place at the 
Large Data Facility, which will also serve as a data access center
for the US community.
The other half of the data release processing will be done at
CC-IN2P3. 
...
it is not pushing the expected technology to the limit, reducing
...
standards. The primary implementation language is Python and, where
...
\caption{
  ...
  about 27.5, roughly equivalent to 10-year LSST depth.
  }
...
has been used to reduce data taken with the HSC camera (Miyazaki et
al.~2017) on
...
%Miyazaki et al 2017 is in press in PASJ, but was not posted to
%arXiv.  It still needs a bibtex entry. 
...
columns, the brighter-fatter effect, and QE variations.
