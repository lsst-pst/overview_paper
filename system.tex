...
%arXiv:0912.0201}
(\cite{SciBook}; hereafter SciBook),
...
\cite{Percival2010}; \cite{LSSTDESC}; \cite{Weinberg2013}, and
...
energy (see e.g. Zhan 2006, and references therein). The challenge is to turn this available precision into accuracy, by careful modeling and marginalization over a variety of systematic effects (see e.g. \cite{Krause2017}).
...
galaxy clusters (see e.g., \cite{Dawson2012}) and mass density profiles in
...
X-ray emission; \cite{Newman2013} and \cite{Rocha2012}), dwarf satellite
galaxies (both their abundance and gamma ray emission; see e.g., \cite{Hargis2014}
and \cite{DrlicaWagner2015}),  the subtle perturbations of stellar
streams (\cite{Belokurov2016}), and massive compact halo object
microlensing (\cite{Alcock2001}).
...
the expansion of the Universe and the growth of structure (e.g., \cite{Hu1999};
for recent reviews see \cite{Kilbinger2015} and \cite{Mandelbaum2017}).  Measurements of cosmic shear as a function of
...
measure the shapes of galaxies (\cite{Tyson2008a};
\cite{Jee2011}).

Type Ia supernovae provided the first robust evidence that the expansion of the
Universe is accelerating (\cite{Riess1998}; \cite{Perlmutter1999}). To fully
...
bandpasses, must be accurate to about 1\% (\cite{WoodVasey2007}),
...
(\cite{Garnavich2005}; \cite{Pinto2005}; \cite{SciBook} Ch.~11). The added statistical leverage
...
proposed surveys, such as Euclid (\cite{Laureijs2011}) and WFIRST
(\cite{Spergel2015}), which will provide
%\footnote{http://wfirst.gsfc.nasa.gov}
wide-field optical-IR imaging from space;
DESI (\cite{Levi2013})
%\footnote{http://desi.lbl.gov}
and PFS (\cite{Takada2014a}), which will measure
...
mass distribution -- as probed by neutral hydrogen in CHIME (\cite{Newburgh2014}), HIRAX (\cite{Newburgh2016})
...
Current and future ground-based CMB experiments, such as Advanced ACT (\cite{DeBernardis2016}),
SPT-3G (\cite{Benson2014}), Simons Observatory, and CMB Stage-4 (\cite{Abazajian2016}), will also offer invaluable opportunities for
...
and robust data processing system (\cite{Ivezic2007a}). The images should reach a depth of at
...
discover rare distant TNOs such as Sedna (\cite{Brown2004})
and 2012 VP113 (\cite{Trujillo2014}).
...
(\cite{SciBook} Ch.~8; \cite{Law2009}; \cite{Djorgovski2012};
\cite{Rowe2014}).
...
disruptions by black holes (\cite{Bloom2011}; \cite{Gezari2012b}),
and possibly new classes of transients, such as binary mergers of
black holes (\cite{Shields2008}; \cite{Thornton2013}) and chaotic
eruptions (\cite{Arnett2011}), and, further yet, completely
unexpected phenomena.

Such a survey would likely detect microlensing by stars and compact objects in
the Milky Way, but also in the Local Group and perhaps beyond (\cite{deJong2008}).
...
measure their mass (\cite{Gould1992}). It would open the possibility of
discovering populations of binaries and planets via transits (e.g., \cite{Beaulieu2006};
\cite{Drake2010}; \cite{Choi2013}; \cite{Batista2014}), as well as
...
subtypes of core collapse SN, \cite{Bianco2014}.)
...
ultra-compact double degenerate binary systems (\cite{Anderson2005}),
to constrain the properties of fast faint transients (such as optical
flashes associated with gamma-ray bursts; \cite{Bloom2008}), to
detect electromagnetic counterparts to gravitational waves (\cite{Nissanke2013},
\cite{Scolnic2017}) and to further constrain the
properties of new classes of transients discovered by programs such as
the Deep Lens Survey (\cite{Becker2004}), the Catalina Real-time
Transient Survey (\cite{Drake2009}) and the Palomar Transient Factory
(\cite{Law2009}), the Zwicky Transient Factory (\cite{Bellm2014}). Observations
over a decade will enable the study of long period variables, intermediate mass
black holes, and quasars (\cite{Kaspi2007}; \cite{MacLeod2010}; \cite{Graham2014};
\cite{Chapline2016}).
...
giant stars (e.g., \cite{Helmi2003}) as well as to identify variable
stars such as RR Lyrae (\cite{Sesar2010}, \cite{Sharma2011}),
and astrometric precision of about 10 mas per observation to enable parallax and proper motion measurements
(\cite{SciBook} Ch.~6,7). In order to probe the halo out to its presumed edge at $\sim100$ kpc (\cite{Ivezic2003})
...
\cite{Majewski2003}, \cite{Chou2007}) and other halo substructures
($\sim 30$ kpc, \cite{Carollo2007}), yielding new insights into how
they formed.  These
...
accuracy should be 0.2 mas yr$^{-1}$ or better. This is the same accuracy as will be delivered by the recently launched Gaia mission\footnote{http://sci.esa.int/gaia/} (\cite{Perryman2001}; \cite{deBruijne2012}) at its faint limit ($r \sim 20$).
...
about the outer scale correction see \cite{Tokovinin2002}. The observed distribution
...
\item  {\it The filter complement} should include at least six filters
    ...
    SDSS-like $u$ band (\cite{Fukugita1996}) is extremely important for separating
...
and galaxies have $k<0.5$. For more details, please see \cite{Nemiroff2003}.}.
...
We note that the Pan-STARRS project (\cite{Kaiser2002}), with similar science
...
Digital Sky Survey
(SDSS) system (\cite{Fukugita1996}) because of its demonstrated success in a wide
variety of applications, including photometric redshifts of galaxies (\cite{Budavari2003}),
separation of stellar populations (\cite{Lenz1998}; \cite{Helmi2003}),
and photometric selection of quasars (\cite{Richards2002}; \cite{Ross2012}). The extension of the
...
simultaneously fitting a three-parameter stellar model SED (\cite{Kurucz1979}) and
six physical parameters of a sophisticated atmospheric model (MODTRAN, \cite{Anderson1999})
...
Fig.~\ref{Fig:modtran2}. Adapted from \cite{Burke2007}.}
\label{Fig:modtran1}
\end{figure}
...
The atmosphere model (MODTRAN, \cite{Anderson1999}) includes six
...
The overall photometric calibration philosophy (\cite{Stubbs2006}) is to measure explicitly, at 1 nm resolution, the
...
as a function of wavelength  (\cite{Stubbs2007};
...
This calibration process has been undertaken by the Dark Energy Survey (DES) team, with results reported in \cite{Burke2017}).
...
(\cite{Padmanabhan2008}; \cite{Schlafly2012}, \cite{Burke2017}), although measurements with ground-based telescopes
...
sufficient accuracy (\cite{Ivezic2007b}), as long as high-quality
...
Astrometric calibration will be based on the results from the Gaia mission (\cite{Gaia2016}), which will provide
...
Paul-Baker Mersenne-Schmidt system; \cite{Angel2000}) with a very fast $f$/1.234 beam. The optical
...
and 2009.  The primary-tertiary mirror was polished by the Richard F.Caris Mirror Lab at the University of Arizona Tucson before being inspected and accepted by LSST in April 2015 \cite{Araujo2016}. The  primary-tertiary mirror cell was fabricated by CAID in Tucson and is undergoing acceptance tests. The integration of the actuators and final tests with the mirror is scheduled later this fall and beginning of 2018.
...
\cite{Mamajek2012}).  The telescope enclosure houses a compact, stiff
...
and SDSS (\cite{Abazajian2009}). As an example,
...
(including descoping), and trade studies can be evaluated (\cite{Connolly2014}).
...
The LSST Operations Simulator (\cite{Delgado2014}) was developed to enable a
...
twilight at each observation (\cite{Yoachim2014}). The time taken to move from one observation to
...
\cite{Jones2014}). MAF provides tools to analyze the properties of a
...
The simulated astronomical catalogs (CatSim; \cite{Connolly2014}) are
...
sources are based on the Galactic structure models of \cite{Juric2008}
...
spectral energy distribution (SED). \cite{Kurucz1993} model spectra are
...
white dwarf stars are taken from \cite{Bergeron1995}.  SEDs for M,
L, and T dwarfs are generated from a combination of spectral models
and by stacking spectra from the SDSS (e.g., \cite{Cushing2005},
\cite{Bochanski2007}, \cite{Burrows2006}, \cite{Pettersen1989},
\cite{Kowalski2010}). The adopted metallicity for each star is based
on a model from \cite{Ivezic2008a}, and proper motions are
based on the kinematic model of \cite{Bond2010}.  Light curve
...
Galactic model of \cite{Amores2005}. To provide consistency with
the modeling of extragalactic fluxes in the simulations, the dust model in the Milky Way integrated
to 100 kpc is re-normalized to match the \cite{Schlegel1998} dust maps.

Galaxy catalogs are derived from the Millennium simulations of de
\cite{DeLucia2006}.  These models extend pure dark matter N-body
...
\cite{Bruzual2003} spectral synthesis models. These fits are
...
scale lengths are taken from \cite{DeLucia2006}. Half-light radii
for bulges are estimated using the empirical absolute-magnitude
vs. half-light radius relation given by \cite{Gonzalez2009}.
...
Quasar/AGN catalogs are generated using the \cite{Bongiorno2007}
luminosity function for $M_B < -15$, over an area of 100
deg$^2$. Their observed SEDs are generated using a composite
rest-frame spectrum derived from SDSS data by \cite{VandenBerk2001}.
...
the results from \cite{Xue2010}.  Each galaxy hosts at most one
...
prescriptions given by \cite{MacLeod2010}.

Asteroids are simulated using the Solar System models of \cite{Grav2007}.
...
are assigned using the C and S type asteroids of \cite{DeMeo2009}.
...
point using the $OpenOrb$ software package (\cite{Granvik2009}).
...
(\cite{Rowe2015}), and Phosim (\cite{Peterson2015}). Galsim is a
...
\caption{ A simulated image of a single LSST CCD using PhoSim
  ...
  image is a color composite (\cite{Lupton2004}) from a set of 30
  second $gri$ visits.}
