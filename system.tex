...
For example, the SDSS, with its 2.5-m telescope (\cite{2006AJ....131.2332G}) and a
camera with 30 imaging CCDs (\cite{1998AJ....116.3040G}), has an effective \'etendue of
only 5.9 m$^2$ deg$^2$.
...
%arXiv:0912.0201}
(\cite{2009arXiv0912.0201L}; hereafter SciBook),
...
energy to match observational constraints (\cite{2007ApJ...659...98R}; \cite{2009ApJS..180..330K};
\cite{2010MNRAS.401.2148P}; \cite{2012arXiv1211.0310L}; \cite{Weinberg2013}, and
...
energy (see e.g. Zhan 2006, and references therein). The challenge is to turn this available precision into accuracy, by careful modeling and marginalization over a variety of systematic effects (see e.g. \cite{2017MNRAS.470.2100K}).
...
galaxy clusters (see e.g., \cite{2012ApJ...747L..42D}) and mass density profiles in
...
X-ray emission; \cite{2013ApJ...765...24N} and \cite{Rocha2012}), dwarf satellite
galaxies (both their abundance and gamma ray emission; see e.g., \cite{2014ApJ...795L..13H}
and \cite{2015ApJ...809L...4D}),  the subtle perturbations of stellar
streams (\cite{2016MNRAS.456..602B}), and massive compact halo object
microlensing (\cite{2001ApJ...550L.169A}).
...
the expansion of the Universe and the growth of structure (e.g., \cite{1999ApJ...514L..65H};
for recent reviews see \cite{2015RPPh...78h6901K} and \cite{2017arXiv171003235M}).  Measurements of cosmic shear as a function of
...
measure the shapes of galaxies (\cite{2008ASPC..394..107T};
\cite{2011PASP..123..596J}).

Type Ia supernovae provided the first robust evidence that the expansion of the
Universe is accelerating (\cite{1998AJ....116.1009R}; \cite{1999ApJ...517..565P}). To fully
...
bandpasses, must be accurate to about 1\% (\cite{2007ApJ...666..694W}),
...
(\cite{2004AAS...20510818G}; \cite{2004AAS...20510820P}; \cite{2009arXiv0912.0201L} Ch.~11). The added statistical leverage
...
proposed surveys, such as Euclid (\cite{2011arXiv1110.3193L}) and WFIRST
(\cite{2015arXiv150303757S}), which will provide
%\footnote{http://wfirst.gsfc.nasa.gov}
wide-field optical-IR imaging from space;
DESI (\cite{2013arXiv1308.0847L})
%\footnote{http://desi.lbl.gov}
and PFS (\cite{2014PASJ...66R...1T}), which will measure
...
mass distribution -- as probed by neutral hydrogen in CHIME (\cite{2014SPIE.9145E..4VN}), HIRAX (\cite{2016SPIE.9906E..5XN})
...
Current and future ground-based CMB experiments, such as Advanced ACT (\cite{2016SPIE.9910E..14D}),
SPT-3G (\cite{2014SPIE.9153E..1PB}), Simons Observatory, and CMB Stage-4 (\cite{2016arXiv161002743A}), will also offer invaluable opportunities for
...
and robust data processing system (\cite{2007IAUS..236..353I}). The images should reach a depth of at
...
discover rare distant TNOs such as Sedna (\cite{2004ApJ...617..645B})
and 2012 VP113 (\cite{2014Natur.507..471T}).
...
(\cite{2009arXiv0912.0201L} Ch.~8; \cite{2009PASP..121.1395L}; \cite{2012IAUS..285..141D};
\cite{2014ApJ...784...45R}).
...
disruptions by black holes (\cite{2011Sci...333..203B}; \cite{2012Natur.485..217G}),
and possibly new classes of transients, such as binary mergers of
black holes (\cite{2008ApJ...682..758S}; \cite{2013Sci...341...53T}) and chaotic
eruptions (\cite{2011ApJ...741...33A}), and, further yet, completely
unexpected phenomena.

Such a survey would likely detect microlensing by stars and compact objects in
the Milky Way, but also in the Local Group and perhaps beyond (\cite{2008A&A...478..755D}).
...
measure their mass (\cite{1992ApJ...392..442G}). It would open the possibility of
discovering populations of binaries and planets via transits (e.g., \cite{2006Natur.439..437B};
\cite{2010arXiv1009.3048D}; \cite{2013ApJ...768..129C}; \cite{2014ApJ...780...54B}), as well as
...
subtypes of core collapse SN, \cite{2014ApJS..213...19B}.)
...
ultra-compact double degenerate binary systems (\cite{2005AJ....130.2230A}),
to constrain the properties of fast faint transients (such as optical
flashes associated with gamma-ray bursts; \cite{2008AN....329..284B}), to
detect electromagnetic counterparts to gravitational waves (\cite{2013ApJ...767..124N},
\cite{2017arXiv171005845S}) and to further constrain the
properties of new classes of transients discovered by programs such as
the Deep Lens Survey (\cite{2004ApJ...611..418B}), the Catalina Real-time
Transient Survey (\cite{2009ApJ...696..870D}) and the Palomar Transient Factory
(\cite{2009PASP..121.1395L}), the Zwicky Transient Factory (\cite{2014htu..conf...27B}). Observations
over a decade will enable the study of long period variables, intermediate mass
black holes, and quasars (\cite{2007ApJ...659..997K}; \cite{2010ApJ...721.1014M}; \cite{2014MNRAS.439..703G};
\cite{2016JCAP...11..042C}).
...
baryon physics becomes important, and the nature of dark matter may manifest itself in observable ways (e.g., \cite{Weinberg2013}).
...
giant stars (e.g., \cite{2003ApJ...586..195H}) as well as to identify variable
stars such as RR Lyrae (\cite{2010ApJ...708..717S}, \cite{2011ApJ...728..106S}),
and astrometric precision of about 10 mas per observation to enable parallax and proper motion measurements
(\cite{2009arXiv0912.0201L} Ch.~6,7). In order to probe the halo out to its presumed edge at $\sim100$ kpc (\cite{ 2004ASPC..327..104I})
...
\cite{2003ApJ...599.1082M}, \cite{2007ApJ...670..346C}) and other halo substructures
($\sim 30$ kpc, \cite{2007Natur.450.1020C}), yielding new insights into how
they formed.  These
...
accuracy should be 0.2 mas yr$^{-1}$ or better. This is the same accuracy as will be delivered by the recently launched Gaia mission\footnote{http://sci.esa.int/gaia/} (\cite{2001A&A...369..339P}; \cite{2012Ap&SS.341...31D}) at its faint limit ($r \sim 20$).
...
about the outer scale correction see \cite{2002PASP..114.1156T}. The observed distribution
...
\item  {\it The filter complement} should include at least six filters
    ...
    SDSS-like $u$ band (\cite{1996AJ....111.1748F}) is extremely important for separating
...
\enddata
\tablenotetext{a}{Design specification from the Science Requirements Document (SRD) for 5$\sigma$ depths
...
and galaxies have $k<0.5$. For more details, please see \cite{2003AJ....125.2740N}.}.
...
\begin{figure}[t]
\includegraphics[width=\hsize,clip]{coaddedDepth.pdf}
...
We note that the Pan-STARRS project (\cite{2002SPIE.4836..154K}), with similar science
...
Digital Sky Survey
(SDSS) system (\cite{1996AJ....111.1748F}) because of its demonstrated success in a wide
variety of applications, including photometric redshifts of galaxies (\cite{2003ApJ...595...59B}),
separation of stellar populations (\cite{1998ApJS..119..121L}; \cite{2003ApJ...586..195H}),
and photometric selection of quasars (\cite{2002AJ....123.2945R}; \cite{2012ApJS..199....3R}). The extension of the
...
\begin{figure}
\includegraphics[width=1.0\hsize,clip]{filters_y4.pdf}
...
\begin{figure}
\includegraphics[width=1.0\hsize,clip]{modtran1.pdf}
\caption{An example of determination of the atmospheric opacity by
simultaneously fitting a three-parameter stellar model SED (\cite{1979ApJS...40....1K}) and
six physical parameters of a sophisticated atmospheric model (MODTRAN, \cite{1999SPIE.3866....2A})
...
Fig.~\ref{Fig:modtran2}. Adapted from \cite{2007AAS...21113723B}.}
\label{Fig:modtran1}
\end{figure}
...
The atmosphere model (MODTRAN, \cite{1999SPIE.3866....2A}) includes six
...
Adapted from \cite{2007AAS...21113723B}.}
\label{Fig:modtran2}
\end{figure}
...
The overall photometric calibration philosophy (\cite{2006ApJ...646.1436S}) is to measure explicitly, at 1 nm resolution, the
...
as a function of wavelength  (\cite{2007PASP..119.1163S};
...
This calibration process has been undertaken by the Dark Energy Survey (DES) team, with results reported in \cite{2017arXiv170601542B}).
...
(\cite{2008ApJ...674.1217P}; \cite{2012ApJ...756..158S}, \cite{2017arXiv170601542B}), although measurements with ground-based telescopes
...
sufficient accuracy (\cite{2007AJ....134..973I}), as long as high-quality
...
to monitor stability and uniformity of photometric data. We expect to use \cite{2016A&A...595A...2G} photometry, utilising
...
Astrometric calibration will be based on the results from the Gaia mission (\cite{2016A&A...595A...2G}), which will provide
...
Paul-Baker Mersenne-Schmidt system; \cite{2000ASPC..195...81A}) with a very fast $f$/1.234 beam. The optical
...
and 2009.  The primary-tertiary mirror was polished by the Richard F.Caris Mirror Lab at the University of Arizona Tucson before being inspected and accepted by LSST in April 2015 \cite{2016SPIE.9906E..0LA}. The  primary-tertiary mirror cell was fabricated by CAID in Tucson and is undergoing acceptance tests. The integration of the actuators and final tests with the mirror is scheduled later this fall and beginning of 2018.
...
\cite{2012arXiv1210.1616M}).  The telescope enclosure houses a compact, stiff
...
and SDSS (\cite{2009ApJS..182..543A}). As an example,
...
(including descoping), and trade studies can be evaluated (\cite{2014SPIE.9150E..14C}).
...
The LSST Operations Simulator (\cite{2014SPIE.9150E..15D}) was developed to enable a
...
twilight at each observation (\cite{2016SPIE.9910E..1AY}). The time taken to move from one observation to
...
\cite{2014SPIE.9149E..0BJ}). MAF provides tools to analyze the properties of a
...
The simulated astronomical catalogs (CatSim; \cite{2014SPIE.9150E..14C}) are
...
sources are based on the Galactic structure models of \cite{2008ApJ...673..864J}
...
spectral energy distribution (SED). \cite{1993sssp.book.....K} model spectra are
...
white dwarf stars are taken from \cite{1995PASP..107.1047B}.  SEDs for M,
L, and T dwarfs are generated from a combination of spectral models
and by stacking spectra from the SDSS (e.g., \cite{2005ApJ...623.1115C},
\cite{2007AJ....133..531B}, \cite{2006ApJ...640.1063B}, \cite{1989A&A...217..187P},
\cite{2010ApJ...714L..98K}). The adopted metallicity for each star is based
on a model from \cite{2008ApJ...684..287I}, and proper motions are
based on the kinematic model of \cite{2010ApJ...716....1B}.  Light curve
...
Galactic model of \cite{2005AJ....130..659A}. To provide consistency with
the modeling of extragalactic fluxes in the simulations, the dust model in the Milky Way integrated
to 100 kpc is re-normalized to match the \cite{1998ApJ...500..525S} dust maps.

Galaxy catalogs are derived from the Millennium simulations of de
\cite{2006MNRAS.366..499D}.  These models extend pure dark matter N-body
...
\cite{2003MNRAS.344.1000B} spectral synthesis models. These fits are
...
scale lengths are taken from \cite{2006MNRAS.366..499D}. Half-light radii
for bulges are estimated using the empirical absolute-magnitude
vs. half-light radius relation given by \cite{2009MNRAS.397.1254G}.
...
Quasar/AGN catalogs are generated using the \cite{2007A&A...472..443B}
luminosity function for $M_B < -15$, over an area of 100
deg$^2$. Their observed SEDs are generated using a composite
rest-frame spectrum derived from SDSS data by \cite{2001AJ....122..549V}.
...
the results from \cite{2010ApJ...720..368X}.  Each galaxy hosts at most one
...
prescriptions given by \cite{2010ApJ...721.1014M}.

Asteroids are simulated using the Solar System models of \cite{2007AAS...211.4721G}.
...
are assigned using the C and S type asteroids of \cite{2009Icar..202..160D}.
...
point using the $OpenOrb$ software package (\cite{2009M&PS...44.1853G}).
...
(\cite{2015A&C....10..121R}), and Phosim (\cite{2015ApJS..218...14P}). Galsim is a
...
\begin{figure*}
\centerline{\includegraphics[width=0.98\hsize,clip]{chip2014.jpg}}
%\vskip -1in
\caption{ A simulated image of a single LSST CCD using PhoSim
  (covering a $13.3\times13.3$ arcmin$^2$ region of the sky). The
  image is a color composite (\cite{2004PASP..116..133L}) from a set of 30
  second $gri$ visits.}
