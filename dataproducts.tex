...
prompt and data release products, more specialized
investigations may require additional custom, user-created, products
derived from the LSST data. We will make it possible for the end-users
to create (or use) such {\bf added value\footnote{Formerly know as ``Level 3 Data Products``.}}
products at the LSST Large Data Facility. 
To enable these use cases, the LSST DM system will devote the equivalent of 10\% of its
processing and storage capabilities to creation, use, and federation
of added-value (user-created) data products. This includes allowing the
end-users to use the LSST database infrastructure to store and
share their results (e.g., added value catalogs).

To further enable user-driven added value (Level 3) processing, the LSST software
...
more rapidly develop custom processing codes, leveraging 15+
