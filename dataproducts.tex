...
SDSS experience (\cite{Sesar2007}), the random photometric error for
point sources, as
...
activity (which influences the night sky brightness, \cite{Patat2008}) is about
0.1-0.2 mag.
...
described in \cite{Jones2017}, \cite{Veres2017a,Veres2017b}, and \cite{Grav2016}.
...
Here we briefly summarize the LSST project analysis carried out in \cite{Jones2017}; this
...
System model of of \cite{Grav2007}.
...
spectra (\cite{DeMeo2009}), combined with the LSST bandpasses (listed in Table~\ref{tab:eqparams}),
...
Object Processing System (MOPS) code (\cite{Veres2017a,Veres2017b}; \cite{Jedicke2005}) developed by the
...
$>$99\% linking efficiency across all classes of Solar System objects (\cite{Denneau2013}),
and at least 93\% efficiency for NEOs (\cite{Veres2017a,Veres2017b}).

The LSST baseline cadence discovers 66\% of PHAs with $H\leq22$ (equivalent to $D\ge140$~m)
after 10 years of operations (61\% of NEOs with $H\leq22$) (\cite{Jones2017}).  This cadence spends 6\% of the total
...
effects) at the level of 20-30 mas (measured from repeated scans; \cite{Pier2003});
...
techniques developed at the U.S. Naval Observatory (\cite{Monet2003}). The expected
proper motion and
...
accuracy of $\sim$5 mas yr$^{-1}$ per coordinate at $r=20$ (\cite{Munn2004}). Gaia
is expected to deliver parallax errors of 0.3 mas and proper motion errors of
0.2 mas yr$^{-1}$ at its faint end at $r\sim20$ (\cite{Perryman2001}). Hence, LSST will smoothly
...
data-intensive astronomy (\cite{Tyson2008b}; \cite{Ivezic2008b};
\cite{Bloom2008}; \cite{Borne2008}; \cite{Ivezic2014}). To address these questions, the statistics and machine-learning research
...
colorized following \cite{Lupton2004}.  The depth gain for the bottom image
...
\cite{Morokuma2007}); the bottom image was taken with Suprime-cam at Subaru.
Adapted from \cite{Blandford2008}.}
\label{Fig:panels2}
\end{figure}
