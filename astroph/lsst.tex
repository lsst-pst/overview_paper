...
\slugcomment{A living LSST document (arXiv:0805.2366); version 2.0.9 of June 4, 2011}
...
\begin{abstract}
\R{In the history of astronomy,} Major advances in our understanding of the
...
the optical, the Large Synoptic Survey Telescope (LSST). LSST will have 
unique survey capability in the faint time domain. The LSST design is 
...
Cerro Pach\'{o}n in Northern Chile. The current baseline design, with an 8.4m 
(6.7m effective) primary mirror, a 9.6 deg$^2$ field of view, and a 3.2 Gigapixel 
...
and photometric accuracy. The \B{total} survey area will include 30,000 deg$^2$ with 
$\delta<+34.5^\circ$, and will be imaged multiple times in six bands, $ugrizy$, 
covering the wavelength range 320--1050 nm. The project is scheduled to 
...
region about 1000 times (summed over all six bands) during the anticipated 10 years 
of operations, and yield a coadded map to $r\sim27.5$. These data will result in 
databases including 10 billion galaxies and a similar number of stars, and will 
...
The goal is to make LSST data products available to the public and scientists around the 
...
requirement to avoid large airmasses ($X<1.5$, where approximately
$X={\rm sec}(\theta)$ and $\theta$ is the zenith distance), which would substantially 
...
\begin{figure}[t!]
%\hskip -1.7in
\includegraphics[width=1.0\hsize,angle=0.0,clip]{camera2009.ps}
%\vskip -2.5in
...
\begin{figure*}[t!]
%\hskip 0.3in
\includegraphics[width=1.0\hsize,clip]{DMp3.ps}
...
\begin{figure}
%\hskip -0.8in
\includegraphics[width=1.0\hsize,clip]{panels1_2.ps}
