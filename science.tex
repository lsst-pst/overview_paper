...
% $l< 100$, and intrinsic ellipticities which dominate at $l> 1000$ (\cite{Zhan2006a}).}
% \label{Fig:wlPk}
% \end{figure}
...
%oscillations (\cite{Eisenstein2005}; \cite{Cole2005}; \cite{Anderson2014}). LSST will measure the
...
\cite{Cooray2001}; \cite{Blake2003}; \cite{Hu2003a}; \cite{Linder2003}; \cite{Seo2003}).
...
%result based on WMAP and SDSS data (\cite{Komatsu2009}; \cite{Percival2010}).
...
gravitational potential of structure through inferences of their peculiar velocities (\cite{Gordon2007},
\cite{Bhattacharya2011}, \cite{Howlett2017}), weak lensing of supernova brightnesses (\cite{Dodelson2006},
\cite{Quartin2014}, \cite{Macaulay2016}, \cite{Scovacricchi2017}), and the local bulk flow (\cite{Reiss1999},
\cite{Dai2011}, \cite{Turnbull2012}, \cite{Feindt2013},  \cite{Huterer2015}) as well as
low redshift constraints on the isotropy of the universe (\cite{Javanmardi2015}, \cite{Antoniou2010},
\cite{Campanelli2011}, \cite{Cai2013}, \cite{Colin2011}).  2) The rapidly sampled ``mini-survey''
...
For example, WL$+$LSS and SN can separately probe anisotropy of dark energy (e.g. \cite{Zhan2009}).
...
of 0.04 eV or better (\cite{Cooray1999}; \cite{Song2004}; \cite{Hannestad2006}).
%The current best limit, as derived from a combination of WMAP, the SDSS power spectrum,
%SN, and the Ly$\alpha$ forest, is 0.17 eV (\cite{Seljak2006}).
...
\item The clustering properties of those same galaxy clusters will also be used to constrain
  ...
  the mass-observable relation and photometric redshift uncertainties (\cite{Oguri2011}), and to constrain the effects of super-sample covariance in the two-point functions of WL and LSS (\cite{Hu2003b}, \cite{Takada2014b}).
...
%constraint on dark energy (\cite{Wang2005}). LSST will find over 200,000 such
...
LSST will also discover between 500 and 1000 strongly lensed Type Ia supernovae (\cite{Goldstein2017a,Goldstein2017b}), which will provide hundreds of additional high-quality time delays.
...
% the SkyMapper (\cite{Keller2001}) which is flux limited to $r=22.6$
...
%          (e.g., \cite{Charbonneau2000}; \cite{Konacki2003}).
...
and the {\it Fermi}
Gamma-ray Space Telescope (e.g., \cite{Atwood2009}).
